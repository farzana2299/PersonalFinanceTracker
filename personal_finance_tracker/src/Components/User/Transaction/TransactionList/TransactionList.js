...
function TransactionList({ uid }) {
  ...
  const { list: transaction, loading, page, totalPages, filterType, typeValue, startDate, endDate, sortBy } =
  ...
  }, [page, filterType, typeValue, startDate, endDate, sortBy]);

  useEffect(() => {
    if (transactionAdded) {
      getuserTransaction({ page: 1, limit: 10, sortBy });
  ...
  <div className="container">
    <Form className="mb-3">
      <Row className="align-items-center g-2">
        ...
        {filterType && (
          <Col xs={12} sm="auto">
            <Button variant="primary" onClick={() => dispatch(setPage(1))} className="w-100">
              Search
            </Button>
          </Col>
        )}
        ...
        <Col xs={12} sm={6} md={2}>
          <Form.Select
            value={sortBy}
            onChange={(e) => {
              dispatch({ type: 'transactionlist/setSortBy', payload: e.target.value });
              dispatch(setPage(1));
            }}
          >
                ...
                <tr key={item._id}>
                  ...
                  <td>
                    <div className="d-flex justify-content-center">
                      <i
                        className="fa-regular fa-pen-to-square text-primary me-3"
